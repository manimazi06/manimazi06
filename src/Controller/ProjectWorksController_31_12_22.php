    ...
    public function funddetails($id = null, $work_id = null)
   {
    ...
    {        
		...
		if($type == 0){
			...
			$type_cond = " AND psd.detailed_estimate_approval = 0 AND project_work_status_id IN (3)";		
		}else if($type == 1){	
           $title = "Abstract / Tehnical Sanction List";		
           $type_cond = " AND psd.detailed_estimate_approval = 0 AND project_work_status_id IN (5,6,7)";
           //$type_cond = "";
		}else if($type == 2){	
		   ...
		   $technicalSanction = $this->TechnicalSanctions->newEmptyEntity();		     
			$attachment  = $this->request->getData('detailed_estimate_upload');
             ...
             if($this->TechnicalSanctions->save($technicalSanction)){
			        ...
			        $users  = $this->Users->find('all')->where(['Users.role_id'=>$projectWorkSubdetail['approval_role'],'Users.is_active'=>1])->first();
                    $user_id = $users['id'];	
