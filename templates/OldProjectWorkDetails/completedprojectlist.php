...
<div class="col-md-12"><?php //echo 'hi'; exit(); ?>
    <div class="card card-topline-aqua">
        	<div class="card-head">
				<header>Completed Works Details</header>	
				 <?php /*if($role_id == 1){  ?>
				 <div class="tools">
				  <?php echo $this->Html->link(__('Add PCR Work<i class="fa fa-plus"></i>'), ['action' => 'pcradd'], ['escape' => false, 'class' => ' btn btn-info']); ?>
				</div>
				<?php  }*/ ?>						
			</div>
		  <div class="card-body"> 
                                    ...
                                    foreach ($oldProjectWorkDetails as $oldProjectWorkDetail) : ?>
                                        <tr class="odd gradeX">
											...
											<?php if($role_id == 9 || $role_id == 14){ ?>
												...
												<?php if($role_id == 9 || $role_id == 14){ ?>
												    <?php echo $this->Html->link(__('<i class="fa fa-pencil"></i> Edit'), ['action' => 'completedbasicdetail',$oldProjectWorkDetail['id']], ['escape' => false, 'class' => 'btn btn-outline-danger btn-sm']); ?>&nbsp;&nbsp;&nbsp;
												<?php }  }else{ 
												 if($role_id == 9 || $role_id == 14){ ?>												 
												    <?php  echo $this->Html->link(__('<i class="fa fa-pencil"></i> Update'), ['action' => 'completedbasicdetail',$oldProjectWorkDetail['id'],$project[$oldProjectWorkDetail['id']]['project_work_id'],$project[$oldProjectWorkDetail['id']]['id']], ['escape' => false, 'class' => 'btn btn-outline-success btn-sm']); ?><br><br>
    ...
    $(".btn-sweetalert").attr("onclick", "").unbind("click"); //remove function onclick button
	  ...
	  $("#FormID").validate({
        ...
        submitHandler: function(form) {			
			...
			if(fin_id != '' || dist_id != '' || go_no != '' || division_id != ''){           
               form.submit();			 
			}else{
