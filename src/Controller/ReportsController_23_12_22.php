...
class ReportsController extends AppController
{
  ...
  public function sanctionedreport()
  {
	...
	if ($this->request->is(['post', 'patch', 'put'])) {
			...
			if ($status_work == 1) {
				$title = "Sanctioned";
				$type_cond = " AND project_subdetail.project_work_status_id between 1 and 6";
			}
			if ($status_work == 2) {
				$title = "To be Commenced";
				$type_cond = " AND project_subdetail.project_work_status_id between 17 and 19";
			}
			if ($status_work == 3) {
				$title = "Progress";
			...
			if ($status_work == 4) {
				...
				$title = "Completed";
				$type_cond = " AND project_subdetail.project_work_status_id between 7 and 11";
			}
...
if ($division) {
  ...
  /*public function progressreport()
  {
   ...
   public function progressreport()
       ...
       $connection            = ConnectionManager::get('default');
        if ($division && $finance) {
            ...
            foreach ($projects as $project) {
                ...
                $quarter_sql = "SELECT 
							sum(CASE WHEN quarter_details.police_designation_id=1 THEN (quarter_details.no_of_quarters) ELSE 0 END)AS ig,
							sum(CASE WHEN quarter_details.police_designation_id=2 THEN (quarter_details.no_of_quarters) ELSE 0 END)AS dsp,
							sum(CASE WHEN quarter_details.police_designation_id=3 THEN (quarter_details.no_of_quarters) ELSE 0 END)AS insp,
							sum(CASE WHEN quarter_details.police_designation_id=4 THEN (quarter_details.no_of_quarters) ELSE 0 END)AS sub_insp,
							sum(CASE WHEN quarter_details.police_designation_id=5 THEN (quarter_details.no_of_quarters) ELSE 0 END)AS constable
    ...
    $division_table = $this->Divisions->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['Divisions.is_active' => 1])->toArray();
    $financial      = $this->FinancialYears->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['FinancialYears.is_active' => 1])->toArray();
    $this->set(compact('expenditure_details', 'project1', 'quarter_details', 'monitoring_details', 'division', 'division_table', 'projects', 'financial', 'division_wise', 'finance_wise', 'finance', 'div'));
   }
    ...
    public function ajaxphoto($sub_id = NULL)
    {
        ...
        $projectworksubdetails = $this->ProjectWorkSubdetails->find('all')->where(['ProjectWorkSubdetails.id' => $sub_id])->first();
        ...
        $monitoringDetails           = $this->ProjectMonitoringDetails->find('all')->where(['ProjectMonitoringDetails.project_work_subdetail_id' => $sub_id])->last();
        $descri = $monitoringDetails['description'];
        $photo  = $monitoringDetails['photo_upload'];

        $this->set(compact('i', 'photo', 'descri', 'name',  'projectworksubdetails', 'monitoringDetails'));
    }
