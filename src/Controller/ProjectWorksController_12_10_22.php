...
//echo '<pre>';  print_r($projectWorkSubdetails);   exit();
    ...
    public function add()
    {
        ...
        $this->set(compact('circles','projectWork', 'departments', 'financialYears', 'buildingTypes', 'Statuses', 'districts', 'divisions','role_id','schemeTypes'));
    }
   ...
   public function projectapprove($id = null)
   {
    ...
    if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData()); exit();
	 ...
	 $projectWorkSubdetails       = $this->ProjectWorkSubdetails->find('all')->contain(['Divisions','Districts','Circles'])->where(['ProjectWorkSubdetails.project_work_id' => $id])->toArray();
	...
	public function administrativesanctionadd(){  
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 
		   ...
		   if($approved_project_count > 0){
			$approved_sub_projects     = $this->ProjectWorkSubdetails->find('all')->contain(['Districts', 'Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids])->toArray();      
		   }
		}
   ...
   public function ajaxsubproject($id = null,$i = null)
    {
		$this->loadModel('ProjectWorkSubdetails');		 	
	    $projectWorkSubdetail      = $this->ProjectWorkSubdetails->find('all')->contain(['Districts','Divisions','Circles'])->where(['ProjectWorkSubdetails.id' => $id])->first();
        $this->set(compact('i','projectWorkSubdetail','id'));
    }
   ...
   public function financialsanctionadd(){    
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 
			    ...
			    if($approved_project_count > 0){
					$approved_sub_projects     = $this->ProjectWorkSubdetails->find('all')->contain(['Districts', 'Divisions','Circles'])->where(['ProjectWorkSubdetails.project_work_id IN' => $approved_project_ids,'ProjectWorkSubdetails.project_work_status_id'=>5])->toArray();      
				   }
		}		
	    $departments    = $this->Departments->find('list', ['limit' => 200])->all();
        $financialYears = $this->FinancialYears->find('list')->order('id DESC')->all();		
   ...
   {	   
    ...
    public function workview($id = null,$work_id = null)
    {
      ...
      $projectWork = $this->ProjectWorks->get($id, [
	   ...
	   $projectWorkSubdetail        = $this->ProjectWorkSubdetails->find('all')->contain(['Divisions','Circles','ProjectWorkStatuses'])->where(['ProjectWorkSubdetails.id' => $work_id])->first();
  ...
  // echo '<pre>'; print_r($requestcount); exit();
   ...
   // $administrativesanction      = $this->ProjectAdministrativeSanctions->find('all')->contain(['ProjectWorks'])->where(['ProjectAdministrativeSanctions.project_work_id' => $id])->first();
    ...
    'contractor_detail_count','contractor_details','requestcount','fundrequests','projectWorkSubdetailscount','planningcount','planningdetail'));
   ...
   public function projectminutedetailupdate($minute_id = null)
   {
    ...
    {        
		...
		if($type == 1){	
           ...
           $type_cond = " AND psd.detailed_estimate_approval = 0";
		}else if($type == 2){	
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo '<pre>'; print_r($this->request->getData()); exit();		   
			  ...
			  $query              =  "SELECT psd.*,dis.name as district_name,dv.name as division_name,c.name as circle_name,ps.name as work_status
									 ...
									 where project.ce_approved=1 $condition  $fin_year_cond  $dept_cond  $project_cond $district_cond  $type_cond";		
											 
		   $projectWorks        = $connection->execute($query)->fetchAll('assoc'); 
		//echo '<pre>'; print_r($projectWorks); exit();
		}       
