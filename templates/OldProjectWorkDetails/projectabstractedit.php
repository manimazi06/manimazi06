                         ...
                         <div class="form-group">
                            <fieldset>                             
                                <table class="table  table-bordered  order-column" style="max-width:100%;margin-right:1%;">
								  <?php if($abstract_subdetails[0]['item_code'] != ''){  ?>
                                    <thead>
                                        <tr>
									...
									<?php $i= 0; foreach($abstract_subdetails as $key => $abstract){ ?>
                                        <tr class="present_row_in_post">
                                            <td class="trcount">Old Record</td>
											<?php if($abstract['building_item_id'] != 0){  ?>
                                            <td>
                                                <?php echo $abstract['item_code']; ?>
                                            </td>
											<?php }else{ ?>
											<td></td>
											<?php } ?>
                                            <td><?php echo $abstract['item_description']; ?>
                                            </td>
											<td><?php echo $abstract['quantity']; ?>
                                            </td>
											<td>
											   <?php echo $abstract['unit']['name']; ?>
                                            </td>
                                            <td><?php echo $abstract['rate']; ?>
                                            </td>                                            
                                            <td>
											<?php echo $abstract['amount']; ?>
                                            </td>
								  ...
								  <?php } ?>
									...
									<?php $i= 0; foreach($abstract_subdetails as $key => $abstract){ ?>
                                        <tr class="present_row_in_post">
                                            <td class="trcount"> <?php if($abstract_subdetails[0]['item_code'] != ''){  ?>New Record<?php }else{ ?>1.<?php } ?></td>
                                            <td>
                                                <?php echo $this->Form->control('workdetail.'.$key.'.building_item_id', ['class' => 'form-control select2', 'templates' => ['inputContainer' => '{{content}}'], 'label' => false, 'error' => false, 'placeholder' => 'Enter Item Code', 'options' => $newbuildingItems, 'empty' => '-Select-', 'onchange' => 'descriptionid(this.value,0)','data-rule-required'=>true,'data-msg-required'=>'Select Item Code','value'=>$abstract['new_building_item_id']]); ?>
                                                <?php echo $this->Form->control('workdetail.'.$key.'.item_code', ['class' => 'form-control', 'templates' => ['inputContainer' => '{{content}}'], 'label' => false, 'error' => false,'required', 'type' => 'hidden','id'=>'item_code_'.$key.'','value'=>$abstract['item_code']]); ?>
                                            </td>											
                                            <td><?php echo $this->Form->control('workdetail.'.$key.'.item_description', ['class' => 'form-control', 'templates' => ['inputContainer' => '{{content}}'], 'label' => false, 'error' => false, 'placeholder' => 'Enter Description', 'rows' => 5,'id'=>'description_0', 'readonly','value'=>$abstract['new_item_description']]); ?>
											...
											<td>
											   <?php echo $this->Form->control('workdetail.'.$key.'.unit_id', ['class' => 'form-control amount', 'templates' => ['inputContainer' => '{{content}}'], 'label' => false, 'error' => false, 'placeholder' => 'Select Unit','data-rule-required'=>false,'data-msg-required'=>'Select Unit','options'=>$units,'empty'=>'-Select-','value'=>$abstract['unit_id']]); ?>
                                            </td>
